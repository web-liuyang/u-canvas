...
export class Polyline extends Graphic<PolylineOptions> {
	...
	public copyWith(options: CopyWithParameter<PolylineOptions>): Polyline {
		return new Polyline({
			id: this.id,
	...
	public override equals(other: Polyline): boolean {
		return super.equals(other) && this.points.length === other.points.length && this.points.every((point, index) => point[0] === other.points[index][0] && point[1] === other.points[index][1])
	}
}
